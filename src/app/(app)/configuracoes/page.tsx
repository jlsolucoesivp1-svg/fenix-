...
import { Save, Download, Database, Upload, AlertTriangle, Trash2, PlusCircle, Users, KeyRound, Phone, User as UserIcon, Building, Image as ImageIcon, X, Wrench, ShieldCheck, QrCode, Calendar, Music, FileQuote, Package, PackagePlus, Paintbrush, ExternalLink, FileSignature } from 'lucide-react';
...
export default function ConfiguracoesPage() {
  ...
  const handleSqlBackup = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Nao foi possivel exportar o backup SQL.');
      }
      ...
      link.href = url;
      link.download = match?.[1] || `backup-sistema-fenix-${new Date().toISOString().split('T')[0]}.sql`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);

      toast({ title: 'Backup SQL Exportado!', description: 'O download do dump SQL do PostgreSQL foi iniciado.' });
    } catch (error) {
      console.error('SQL backup error:', error);
      toast({ variant: 'destructive', title: 'Erro no Backup SQL', description: 'Nao foi possivel exportar o banco em SQL.' });
    }
  };
            ...
            <div className="p-4 border border-destructive/20 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-destructive/10">
                <div><h4 className="font-semibold text-destructive">Backup do Sistema</h4><p className="text-sm text-destructive/80">Salve uma cópia de segurança completa de todos os dados do sistema.</p></div>
                 <div className="flex-shrink-0 flex items-center gap-2">
                    ...
                    <Button variant="outline" onClick={handleSqlBackup} disabled={!currentUser?.permissions.accessDangerZone}>
                        <Database className="mr-2 h-4 w-4"/>Exportar Banco SQL
                    </Button>
                </div>
            </div>
