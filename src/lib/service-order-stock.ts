...
export const getServiceOrderPartKey = (item: Pick<ServiceOrderItem, 'stockItemId' | 'description'>) =>
...
export const findStockItemForServiceOrderItem = (
  ...
  return stock.find((stockItem) => normalizeText(stockItem.name) === normalizedDescription);
};
...
export const getAvailableStockForDraftItem = (
  ...
  return stockItem.quantity + (originalTotals.get(key) || 0) - (draftTotals.get(key) || 0);
};
...
export const syncServiceOrderStock = ({
  ...
  const canonicalPreviousItems = canonicalizeItems(updatedStock, previousItems);
  const canonicalNextItems = canonicalizeItems(updatedStock, nextItems);
  ...
  for (const key of allKeys) {
    ...
    const referenceItem =
      canonicalNextItems.find((item) => item.type === 'part' && getServiceOrderPartKey(item) === key) ||
      canonicalPreviousItems.find((item) => item.type === 'part' && getServiceOrderPartKey(item) === key);
    ...
    const stockItemIndex = updatedStock.findIndex((stockItem) => {
      ...
      return normalizeText(stockItem.name) === normalizeText(referenceItem.description);
    });
