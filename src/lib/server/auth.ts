...
const decodeLegacyBase64Password = (value: string): string =>
...
const verifyPasswordHash = async (
  ...
  const [prefix, cost, blockSize, parallelization, saltValue, keyValue] =
    hash.split('$');
...
export const sanitizeUser = <T extends User>(
  user: T
): Omit<T, 'password'> => {
...
export const verifyPassword = async (
  ...
  const valid =
    decodeLegacyBase64Password(storedPassword) === password;
...
export const normalizeIncomingPassword = async (
  ...
  if (isLegacyBase64Password(trimmedPassword)) {
    return hashPassword(
      decodeLegacyBase64Password(trimmedPassword)
    );
  }
